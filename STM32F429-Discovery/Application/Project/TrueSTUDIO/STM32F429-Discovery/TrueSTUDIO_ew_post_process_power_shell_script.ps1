...
#
#
# PowerShell script for modification of the TrueSTUDIO project configuration file
...
# the window title
$host.ui.RawUI.WindowTitle = "Embedded Wizard Post Process - updating TrueSTUDIO project configuration"
...
{
  Write-Host TrueSTUDIO project configuration file not found at path:
  Write-Host $Path
  Write-Host
  Read-Host Press Enter to continue | Out-Null
  [Environment]::Exit(6)
...
# Check project id
If (-Not (Select-String -Path $Path -Pattern "com.atollic.truestudio.exe.debug.1984525199" -SimpleMatch -Quiet))
{
  Write-Host This 'Embedded Wizard' post process is configured for the predefined TrueSTUDIO
